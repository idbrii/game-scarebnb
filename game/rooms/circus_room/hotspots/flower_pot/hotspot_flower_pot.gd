...
func _on_click() -> void:
    if I.SoundBloop.can_collect():
        await InteractUtil.approach_and_say("Upon closer inspection, these flowers squirt water in my face.")
        InteractUtil.drop_sound_item(I.SoundBloop, self)
        await C.player.say(" BLOOP! I swear I hear a small laugh from inside the vase, and the word, “Gotcha!”")

    else:
        var generic_lines := [
            "I've washed my face enough times today.",
...
func _on_right_click() -> void:
    await InteractUtil.face_and_say("A large pot filled with brightly-coloured flowers.")
