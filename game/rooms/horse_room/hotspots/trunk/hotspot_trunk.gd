...
func _on_click() -> void:
    await InteractUtil.approach_and_say("The trunk is locked, but upon closer inspection I see no seam for its lid, no latches, no handles.")
    await C.player.say("When I place my palm against its canvas-wrapped surface, it radiates warmth, and I could swear it rises and falls gently, as though [i]breathing.[/i]")
    await C.player.say("Then again, maybe I haven’t had enough water today?")
...
func _on_right_click() -> void:
    InteractUtil.face_and_say("It's probably a very normal trunk.")
