...
#
#   func _process(dt):
#       var move = inputmap.get_vector("move")
#       velocity += move * walk_speed * dt
#       var just_jump = inputmap.is_action_just_pressed("jump")
#       if just_jump:
...
static func is_keyboardmouse_input(event: InputEvent) -> bool:
...
func _init(input_map):
    ...
    for action in input_map.get_or_add("pairs", {}):
        ...
        pairs[action] = q
    for action in input_map.get_or_add("quads", {}):
...
func get_action_strength(action: String, exact: bool = false) -> float:
...
func is_action_just_released(action: String, exact: bool = false) -> bool:
    return Input.is_action_just_released(device_actions[action], exact)


func get_axis(pair: String) -> float:
    var p = pairs[pair]
    return Input.get_axis(p[0], p[1])


func get_vector(quad: String, deadzone: float = -1.0) -> Vector2:
    var q = quads[quad]
